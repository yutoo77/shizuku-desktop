...
declare global {
  interface Window {
    companion: {
      ...
      action(action: 'show'|'hide'|'reset'|'call'|'choose-model'|'quit'|'left'|'right'|'up'|'down'|'move-mode'|'pointer-place'|'cancel-placement'|'size-small'|'size-standard'|'size-large'|'stand'|'sit'|'face-left'|'face-right'|'quiet'|'seat-countdown'|'seat-here'|'save-favorite'|'restore-favorite'|'follow-window'|'follow-countdown'|'stop-following'): Promise<void>;
      getStatus(): Promise<{model: string; error: string; shortcuts: boolean; moving: boolean; pointerPlacing: boolean; placementEscape: boolean; loaded: boolean; scale: number; posture: 'standing' | 'sitting'; facing: 'left' | 'right'; quiet: boolean; hasFavorite: boolean; seatCountdown: number; seating: boolean; placementMessage: string; following: boolean; followCountdown: number; followReady: boolean; followMessage: string}>;
    };
  }
}
