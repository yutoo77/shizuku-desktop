...
try {
  ...
  assert.ok(control);
  const callAccess=await page.evaluate(async()=>{try{await window.companion.action('call');return true;}catch{return false;}});
  assert.equal(callAccess,false);
  assert.equal(await inspect(({globalShortcut})=>globalShortcut.isRegistered('CommandOrControl+Alt+Shift+C')),true);
  await control.getByRole('button',{name:'呼ぶ',exact:true}).click();
  await page.waitForFunction(()=>window.__diagnostics.reacting);
  await page.waitForFunction(()=>window.__diagnostics.reactionProgress>0.2);
  const reactionProgress=await page.evaluate(()=>window.__diagnostics.reactionProgress);
  await inspect(()=>{for(let i=0;i<8;i++)void globalThis.__shizuku.action('call');});
  assert.ok(await page.evaluate(()=>window.__diagnostics.reactionProgress)>=reactionProgress);
  await page.waitForFunction(()=>!window.__diagnostics.reacting,null,{timeout:4000});
  assert.equal(await page.evaluate(()=>window.__diagnostics.reactionProgress),1);
  assert.equal(await inspect(()=>globalThis.__shizuku.avatar().isFocusable()),false);
  results.push('Call button starts a finite response; repeated calls do not restart or queue; avatar stays nonfocusable; renderer cannot call itself');

  await inspect(()=>globalThis.__shizuku.setVisible(false));
  await delay(100);
  await inspect(()=>globalThis.__shizuku.trayMenu().items.find(i=>i.label==='呼ぶ').click());
  await page.waitForFunction(()=>window.__diagnostics.visible && window.__diagnostics.reacting);
  assert.equal(await inspect(()=>globalThis.__shizuku.avatar().isVisible()),true);
  await inspect(()=>globalThis.__shizuku.setVisible(false));
  await page.waitForFunction(()=>!window.__diagnostics.visible && !window.__diagnostics.reacting);
  const cancelledCallFrames=await page.evaluate(()=>window.__diagnostics.renderedFrames);
  await delay(250);
  assert.equal(await page.evaluate(()=>window.__diagnostics.renderedFrames),cancelledCallFrames);
  await inspect(()=>globalThis.__shizuku.setVisible(true));
  await delay(100);
  assert.equal(await page.evaluate(()=>window.__diagnostics.reacting),false);
  await inspect(()=>globalThis.__shizuku.avatar().webContents.send('avatar:called',Date.now()-1));
  await delay(100);
  assert.equal(await page.evaluate(()=>window.__diagnostics.reacting),false);
  results.push('Tray call reveals a hidden avatar; hiding cancels without frames or replay; expired call events are ignored');
  ...
  assert.equal(await page.evaluate(()=>window.__diagnostics.reacting),false);
  await inspect(()=>globalThis.__shizuku.action('call'));
  await page.waitForFunction(()=>window.__diagnostics.reacting && !window.__diagnostics.moving);
  assert.equal(await inspect(()=>globalThis.__shizuku.moveState().active),false);
  ...
  await page.waitForFunction(()=>window.__diagnostics.loaded && !window.__diagnostics.reacting && window.__diagnostics.reactionProgress===0);
  results.push('Move and call cancel each other safely; stale move shape is rejected; reloading clears the response');

  await page.emulateMedia({reducedMotion:'reduce'});
  await delay(100);
  await inspect(()=>globalThis.__shizuku.action('call'));
  await page.waitForFunction(()=>window.__diagnostics.reacting);
  const stillCallFrames=await page.evaluate(()=>window.__diagnostics.renderedFrames);
  await delay(350);
  assert.equal(await page.evaluate(()=>window.__diagnostics.renderedFrames),stillCallFrames);
  assert.equal(await page.evaluate(()=>window.__diagnostics.animating),false);
  await page.waitForFunction(()=>!window.__diagnostics.reacting,null,{timeout:3500});
  assert.equal(await page.evaluate(()=>window.__diagnostics.renderedFrames),stillCallFrames+1);
  await delay(150);
  assert.equal(await page.evaluate(()=>window.__diagnostics.renderedFrames),stillCallFrames+1);
  await page.emulateMedia({reducedMotion:'no-preference'});
  results.push('Reduced-motion call uses a still expression and one restoration frame, with no recurring render timer');
